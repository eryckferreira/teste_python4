...
# APRENDENDO A UTILIZAR WHILE LOOP NO PYTHON!
...
# As instruções representam todas as instruções recuadas pelo mesmo número de espaços de caracteres depois que uma construção de programação é
# considerada parte de um único bloco de código.
# Python usa endentação como seu método de agrupamento de declarações.
# Quando um loop while é executado, expr é avaliado primeiro em um contexto booleano e, se for verdadeiro, o corpo do loop é executado.
#  Em seguida, a expr é verificada novamente, se ainda for verdadeira, o corpo é executado novamente e continua até que a expressão se torne falsa.
